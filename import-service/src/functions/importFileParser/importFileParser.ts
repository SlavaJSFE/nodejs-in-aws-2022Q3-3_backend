...
export const handler = async (event: S3Event) => {
  ...
  try {
    ...
    const copiedFile = await s3Client.send(new CopyObjectCommand(copyParams));
    const deletedFile = await s3Client.send(new DeleteObjectCommand(params));

    console.log(parsedProducts, copiedFile, deletedFile);
  } catch (error) {
